...
- ⚠️ **원격 푸시(정산일 알림 · `F-FAM-10`)만은 Expo Go 에서 안 나온다.** SDK 53 부터 안드로이드 Expo Go 는
  푸시 토큰을 안 준다 — 정산일 저장까지만 되고 알림은 안 온다. 알림은 Firebase 키를 넣은 APK 에서만 본다 (README 「푸시 알림」).
- adb 먹통 · 에뮬레이터 프레임버퍼 정지 · 한글 입력 불가 등 함정은 `docs/개발-노트.md` 에 정리돼 있다
  (**gitignore 대상** — 로컬 경로가 들어 있어 공개 저장소에 두지 않는다. 없으면 그 PC 기준으로 새로 적는다).
