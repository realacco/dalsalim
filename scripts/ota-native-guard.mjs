 ...
 * 모르는 것은 막는 쪽이 맞다.
 *
 * ⚠️ **아직 안 보는 것: package-lock.json.** 범위가 `~`/`^` 라 `npm update` 로 락파일만
 * 바뀌어도 실제 설치되는 네이티브 모듈 버전이 달라지는데, package.json 이 그대로면
 * 여기는 조용하다. app.json 만 보던 구멍과 같은 축이다. 닫으려면 락파일의 직접 의존성
 * 해석 버전까지 비교해야 한다.
 *
