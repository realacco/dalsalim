...
import { CATEGORIES, type Category, type FixedExpense, type FixedExpenseGroups } from '../../src/api/types';
...
export default function FixedScreen() {
  ...
  const groups = useQuery({
    queryKey: ['fixed', familyId],
    queryFn: () => api<FixedExpenseGroups>(`/families/${familyId}/fixed-expenses`),
  ...
  const save = useMutation({
    mutationFn: async (value: Draft) => {
      ...
      if (value.id) {
        return api(`/fixed-expenses/${value.id}`, { method: 'PATCH', body });
      }
      return api(`/families/${familyId}/fixed-expenses`, {
        method: 'POST',
        body: { ...body, membershipId: value.membershipId },
      });
    },
    onSuccess: () => {
      setDraft(null);
      setError(null);
      void queryClient.invalidateQueries({ queryKey: ['fixed', familyId] });
  ...
  const remove = useMutation({
    mutationFn: (id: string) => api(`/fixed-expenses/${id}`, { method: 'DELETE' }),
    onSuccess: () => {
      setDraft(null);
      void queryClient.invalidateQueries({ queryKey: ['fixed', familyId] });
      void queryClient.invalidateQueries({ queryKey: ['book', familyId] });
