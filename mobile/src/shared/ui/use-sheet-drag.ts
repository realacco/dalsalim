...
/**
 * 아래 시트를 손가락으로 끌어내려 닫는 **배관**. 지금은 `./sheet` 하나만 쓴다 — 시트를 새로 만들면
 * 이 훅이 아니라 `<Sheet>` 를 쓴다. 배관과 그리는 것을 파일로 가른 것은 아래 함정 주석을 한데 모아두기 위해서다.
 *
 * 여기 모인 것은 **눈에 안 보이는 함정들**이라 복사본이 갈라지면 그 자리에서만 조용히 되살아난다.
 * 아래 주석 넷은 전부 실제로 데인 기록이다 (#18 · #20 · #22).
 *
 * 받은 `translateY` 는 `Animated.View` 에, `drag` 는 머리의 `GestureDetector` 에 건다 (`./sheet`).
 *
