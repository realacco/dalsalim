...
**화면이 걸린 일은 앞에 한 칸이 더 있다.** `/design` 으로 시안과 UX 시나리오를 먼저 그려
승인을 받고 ④ 로 간다 (`.claude/process/start-design.md`). 부를 때만 도는 절차다.
...
절차 본문은 `.claude/process/start-task.md`(시안은 `start-design.md`), 규칙은 `CLAUDE.md` 「작업 흐름」에 있다.
`/task <기능 ID>` 로 ① 부터 자동으로 밟을 수 있고, `/design <기능 ID>` 로 시안을 먼저 그릴 수도,
`/spec <파일 · ID · 도메인>` 으로 조회만 할 수도 있다.
...
5. **동의항목 → 닉네임(`profile_nickname`)만.** 선택 동의로 충분하다 —
   동의하지 않으면 서버가 빈 값으로 받는다. 닉네임은 **가입할 때 한 번만** 카카오에서 받고 그 뒤로는
   앱의 것이다 — 내 정보에서 보고 바꾸며, 가족을 만들거나 참여할 때 「내 이름」 칸의 처음 값이 된다.
   가족 화면에서 사람을 구분하는 건 가족마다 직접 적는 `displayName` 이다
...
| `runtimeVersion` | `"2"` | **어떤 네이티브 런타임이 필요한가** | 네이티브를 실제로 건드렸을 때만 |
| `android.versionCode` | `3` | 안드로이드가 "더 새 것"을 판단하는 정수 | **APK 를 새로 구울 때마다 +1** |
