...
- ⚠️ **`/review` 로 부른 실행은 concurrency 가 취소하지 않는다.** 리뷰가 시작하며 남기는 추적
  코멘트가 또 `issue_comment` 를 일으켜 같은 그룹의 실행을 만드는데, `cancel-in-progress` 가
  켜져 있으면 **리뷰가 자기 코멘트에 죽는다.** job 의 `if` 로는 못 막는다 — concurrency 는
  그보다 먼저, 워크플로 수준에서 적용되기 때문이다.
