...
export async function requestJoin(userId: string, inviteCode: string, displayName: string) {
  ...
  //
  // "ACTIVE 가 하나라도 있나"가 아니라 가족장을 찾는 이유: 막으려는 것은 "사람이 없다"가
  // 아니라 "승인할 사람이 없다"다. 지금은 둘을 가르는 길이 다 막혀 있지만 — 가족장은 넘기지
  // 않고는 못 나가고(F-FAM-08), 탈퇴도 같은 가드에 걸린다(F-SES-08) — 그건 그 두 곳이
  // 지켜주는 것이지 여기서 아는 게 아니다. 예전에 생긴 가족이 이미 그 상태일 수도 있다.
  //
